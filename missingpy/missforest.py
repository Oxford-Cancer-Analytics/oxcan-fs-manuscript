"""MissForest Imputer for Missing Data"""  # fmt: skip
    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    def fit(self, X, y=None, cat_vars=None):
        ...
        if np.any(mask.sum(axis=0) >= (X.shape[0])):
            row_miss = np.all(np.isnan(X), axis=0).nonzero()
            X[:, row_miss] = 0
        ...
        if cat_vars is not None:
            if isinstance(cat_vars, int):
                cat_vars = [cat_vars]
            elif isinstance(cat_vars, list) or isinstance(cat_vars, np.ndarray):
    ...
    def _miss_forest(self, Ximp, mask):
        ...
        if self.num_vars_ is not None:
            ...
            # Reg criterion
            reg_criterion = self.criterion if isinstance(self.criterion, str) else self.criterion[0]
        ...
        if self.cat_vars_ is not None:
            ...
            clf_criterion = self.criterion if isinstance(self.criterion, str) else self.criterion[1]
