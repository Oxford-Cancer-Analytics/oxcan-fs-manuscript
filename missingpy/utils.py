"""Utility Functions"""  # fmt: skip
# Author: Ashim Bhattarai
# License: BSD 3 clause
import numpy as np


def masked_euclidean_distances(X, Y=None, squared=False, missing_values="NaN", copy=True):
    ...
    * John K. Dixon, "Pattern Recognition with Partly Missing Data",
    ...
    X, Y = check_pairwise_arrays(X, Y, accept_sparse=False, force_all_finite=False, copy=copy)
    if np.any(np.isinf(X)) or (Y is not X and np.any(np.isinf(Y))):
    ...
    if np.any(mask_X.sum(axis=1) == X.shape[1]) or (Y is not X and np.any(mask_YT.sum(axis=0) == Y.shape[1])):
        raise ValueError("One or more rows only contain missing values.")

    # else:
    if missing_values not in ["NaN", np.nan] and (np.any(np.isnan(X)) or (Y is not X and np.any(np.isnan(Y)))):
        raise ValueError(f"NaN values present but missing_value = {missing_values}")
    ...
    distances = (X.shape[1] / (np.dot(not_X, not_YT))) * (
        np.dot(X * X, not_YT) - 2 * (np.dot(X, YT)) + np.dot(not_X, YT * YT)
    )
